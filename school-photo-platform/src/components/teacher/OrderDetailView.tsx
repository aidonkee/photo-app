  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'APPROVED_BY_TEACHER':
        return <Badge className="bg-green-100 text-green-700 border-green-200 hover:bg-green-100">Одобрено</Badge>;
      case 'LOCKED':
          ...
          <Card key={item.id} className="overflow-hidden border-slate-200 shadow-sm">
            <CardContent className="p-0">
              <div className="flex flex-col sm:flex-row">
                ...
                {/* === ИЗМЕНЕНИЯ ЗДЕСЬ: Вывод реального фото === */}
                ...
                <div className="p-4 flex flex-col justify-between flex-1">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Badge variant="outline" className="text-slate-600 text-xs whitespace-nowrap ml-2">
                        {item.quantity} шт
                      </Badge>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-2">
                    <span className="text-xs text-slate-400">Цена за шт: {formatCurrency(item.pricePerUnit)}</span>
